...
-- VERSION: 13.5.2 (Wedge Fling Only Update)
...
return function(AccessKey)
    ...
    -- MENGGUNAKAN SCROLLING FRAME UNTUK MENAMPUNG MENU (Canvas disesuaikan)
    ...
    ContentFrame.CanvasSize = UDim2.new(0, 0, 0, 435) -- Ukuran disesuaikan setelah penghapusan slider Fling (410 + 25 = 435)
    ...
    -- TOMBOL & PERINGATAN INFINITE JUMP
    local InfJumpBtn = createBtn("[K] INF JUMP: OFF", UDim2.new(0, 6, 0, 75), UDim2.new(0, 128, 0, 20)); InfJumpBtn.Parent = ContentFrame
    ...
    -- [[ TOMBOL INTERNAL AUTO WEDGE FLING ]]
    local WedgeFlingBtn = createBtn("[V] WEDGE FLING: OFF", UDim2.new(0, 6, 0, 120), UDim2.new(0, 128, 0, 20)); WedgeFlingBtn.Parent = ContentFrame

    -- Elemen-elemen berikutnya digeser 25px dari posisi aslinya
    createLine(UDim2.new(0, 6, 0, 145)).Parent = ContentFrame 
    createLabel("FACES MODE", UDim2.new(0, 6, 0, 151)).Parent = ContentFrame
    local ClassicBtn = createBtn("[G] CLASSIC: OFF", UDim2.new(0, 6, 0, 163), UDim2.new(0, 62, 0, 20)); ClassicBtn.Parent = ContentFrame
    local ProBtn = createBtn("[H] PRO: OFF", UDim2.new(0, 72, 0, 163), UDim2.new(0, 62, 0, 20)); ProBtn.Parent = ContentFrame

    createLine(UDim2.new(0, 6, 0, 189)).Parent = ContentFrame
    createLabel("WALLHOP MODE", UDim2.new(0, 6, 0, 195)).Parent = ContentFrame
    local WHNormalBtn = createBtn("NORMAL", UDim2.new(0, 6, 0, 207), UDim2.new(0, 62, 0, 20), _G.WHNormal and _G.AccentColor or nil); WHNormalBtn.Parent = ContentFrame
    local WHInstantBtn = createBtn("INSTANT", UDim2.new(0, 72, 0, 207), UDim2.new(0, 62, 0, 20)); WHInstantBtn.Parent = ContentFrame

    createLabel("WALLHOP DISTANCE", UDim2.new(0, 6, 0, 232)).Parent = ContentFrame
    local SliderFrame = Instance.new("Frame", ContentFrame); SliderFrame.Size = UDim2.new(0, 128, 0, 12); SliderFrame.Position = UDim2.new(0, 6, 0, 244); SliderFrame.BackgroundColor3 = Color3.fromRGB(25, 25, 30); Instance.new("UICorner", SliderFrame)
    ...
    -- CAMERA SETTINGS
    createLine(UDim2.new(0, 6, 0, 261)).Parent = ContentFrame
    createLabel("CAMERA SETTINGS", UDim2.new(0, 6, 0, 267)).Parent = ContentFrame
    
    local FOVBtn = createBtn("[I] FOV: OFF", UDim2.new(0, 6, 0, 279), UDim2.new(0, 128, 0, 20)); FOVBtn.Parent = ContentFrame

    local FOVSliderFrame = Instance.new("Frame", ContentFrame); FOVSliderFrame.Size = UDim2.new(0, 128, 0, 12); FOVSliderFrame.Position = UDim2.new(0, 6, 0, 304); FOVSliderFrame.BackgroundColor3 = Color3.fromRGB(25, 25, 30); Instance.new("UICorner", FOVSliderFrame)
    ...
    -- FREEZE / LAG SYSTEM
    createLine(UDim2.new(0, 6, 0, 321)).Parent = ContentFrame
    createLabel("FREEZE / LAG SIMULATOR", UDim2.new(0, 6, 0, 327)).Parent = ContentFrame
    local FreezeBtn = createBtn("[O] FREEZE BUTTON: OFF", UDim2.new(0, 6, 0, 339), UDim2.new(0, 128, 0, 20)); FreezeBtn.Parent = ContentFrame

    -- SYSTEM SCALE SETTINGS
    createLine(UDim2.new(0, 6, 0, 364)).Parent = ContentFrame
    createLabel("SCALE SETTINGS", UDim2.new(0, 6, 0, 370)).Parent = ContentFrame
    ...
    UIScaleSliderFrame.Position = UDim2.new(0, 6, 0, 382)
    ...
    ExtScaleSliderFrame.Position = UDim2.new(0, 6, 0, 399)
    ...
    local ExternalWedgeScale -- Objek skala tombol eksternal baru
    ...
    local function syncExtScaleSlider(val)
        ...
        if ExternalWedgeScale then
            ExternalWedgeScale.Scale = val / 100
        end
    end
    ...
    -- ========================================================
    -- [[ 5. TOMBOL EKSTERNAL KHUSUS WEDGE FLING (Sesuai Permintaan) ]]
    -- ========================================================
    local WedgeFlingExternalBtn = Instance.new("TextButton", ScreenGui)
    WedgeFlingExternalBtn.Name = "WedgeFlingExternalButton"
    WedgeFlingExternalBtn.Size = UDim2.new(0, 70, 0, 30)
    WedgeFlingExternalBtn.Position = UDim2.new(0.5, -35, 0.73, 0) -- Terletak sedikit di atas tombol Freeze
    WedgeFlingExternalBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    WedgeFlingExternalBtn.Text = "WEDGE: OFF"
    WedgeFlingExternalBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
    WedgeFlingExternalBtn.Font = Enum.Font.GothamBold
    WedgeFlingExternalBtn.TextSize = 9
    WedgeFlingExternalBtn.Visible = true
    Instance.new("UICorner", WedgeFlingExternalBtn).CornerRadius = UDim.new(0, 5)
    
    local WedgeFlingStroke = Instance.new("UIStroke", WedgeFlingExternalBtn)
    WedgeFlingStroke.Color = Color3.fromRGB(150, 150, 150)
    WedgeFlingStroke.Thickness = 1.5

    -- Sinkronisasi skala eksternal baru
    ExternalWedgeScale = Instance.new("UIScale", WedgeFlingExternalBtn)
    ExternalWedgeScale.Scale = 1.0

    -- Draggable Tombol Eksternal Wedge Fling
    local wf_dragging, wf_dragStart, wf_startPos
    WedgeFlingExternalBtn.InputBegan:Connect(function(i) 
        if (i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch) and not isLocked then 
            wf_dragging = true
            wf_dragStart = i.Position
            wf_startPos = WedgeFlingExternalBtn.Position 
        end 
    end)
    UserInputService.InputChanged:Connect(function(i) 
        if wf_dragging and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then 
            local d = i.Position - wf_dragStart
            WedgeFlingExternalBtn.Position = UDim2.new(wf_startPos.X.Scale, wf_startPos.X.Offset + d.X, wf_startPos.Y.Scale, wf_startPos.Y.Offset + d.Y) 
        end 
    end)
    ...
    -- SINKRONISASI SKALA SLIDER EKSTERNAL AWAL
    syncExtScaleSlider(_G.ExtScaleValue)

    -- DETEKSI OTOMATIS TIKTOK STAIRCASE/WEDGE GLITCH (Hanya Menyisakan Fitur Ini)
    task.spawn(function()
        while true do
            task.wait(0.01)
            if _G.FlingEnabled then
                pcall(function()
                    ...
                    if hrp and hum then
                        -- Lakukan raycast vertikal ke atas sejauh 7.5 studs untuk memeriksa apakah ada tangga/wedge di atas kepala
                        local params = RaycastParams.new()
                        params.FilterDescendantsInstances = {char}
                        params.FilterType = Enum.RaycastFilterType.Exclude
                        
                        local ray = Workspace:Raycast(hrp.Position, Vector3.new(0, 7.5, 0), params)
                        if ray and ray.Instance and ray.Instance.CanCollide then
                            -- Auto spam jump untuk memicu overlap fisika terlempar ke udara
    ...
    -- ========================================================
    -- [[ 6. TOMBOL EKSTERNAL FREEZE (LAG SIMULATOR) ]]
    -- ========================================================
    ...
    -- UIScale objek untuk tombol eksternal
    ...
    local function toggleWedgeFling()
        _G.FlingEnabled = not _G.FlingEnabled
        
        -- Update tombol menu internal
        WedgeFlingBtn.Text = _G.FlingEnabled and "[V] WEDGE FLING: ON" or "[V] WEDGE FLING: OFF"
        WedgeFlingBtn.BackgroundColor3 = _G.FlingEnabled and _G.AccentColor or Color3.fromRGB(30, 30, 35) 
        
        -- Update tombol mengambang eksternal
        WedgeFlingExternalBtn.Text = _G.FlingEnabled and "WEDGE: ON" or "WEDGE: OFF"
        WedgeFlingExternalBtn.BackgroundColor3 = _G.FlingEnabled and _G.AccentColor or Color3.fromRGB(30, 30, 30)
        WedgeFlingStroke.Color = _G.FlingEnabled and _G.AccentColor or Color3.fromRGB(150, 150, 150)
    end
    ...
    -- Hubungkan tombol internal dan tombol eksternal ke fungsi yang sama
    WedgeFlingBtn.MouseButton1Click:Connect(toggleWedgeFling)
    WedgeFlingExternalBtn.MouseButton1Click:Connect(toggleWedgeFling)
    ...
    UserInputService.InputBegan:Connect(function(input, gameProcessed)
        ...
        elseif key == Enum.KeyCode.V then toggleWedgeFling() -- Tombol V sekarang murni menyalakan/mematikan Auto Wedge Fling
        end
    end)
    ...
    print("Louis Hub VIP V13.5.2: Initialized Successfully with Auto Wedge Fling Only.")
end
