...
return function(AccessKey)
    ...
    local FOVBtn = createBtn("[I] FOV: OFF", UDim2.new(0, 6, 0, 259), UDim2.new(0, 128, 0, 20)); FOVBtn.Parent = ContentFrame

    -- SLIDER FOV (1-200)
    local FOVSliderFrame = Instance.new("Frame", ContentFrame); FOVSliderFrame.Size = UDim2.new(0, 128, 0, 12); FOVSliderFrame.Position = UDim2.new(0, 6, 0, 284); FOVSliderFrame.BackgroundColor3 = Color3.fromRGB(25, 25, 30); Instance.new("UICorner", FOVSliderFrame)
    ...
    createLine(UDim2.new(0, 6, 0, 301)).Parent = ContentFrame
    createLabel("FREEZE / LAG SIMULATOR", UDim2.new(0, 6, 0, 307)).Parent = ContentFrame
    local FreezeBtn = createBtn("[O] FREEZE BUTTON: OFF", UDim2.new(0, 6, 0, 319), UDim2.new(0, 128, 0, 20)); FreezeBtn.Parent = ContentFrame
    ...
    UserInputService.JumpRequest:Connect(function()
        ...
        -- Logika Infinite Jump Gratis (Anti-Layang / Mode Ketuk Normal)
        if _G.InfJumpEnabled and canJump then
            local hum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
            if hum then
                local state = hum:GetState()
                -- Hanya izinkan lompatan tambahan saat karakter berada di udara
                if state == Enum.HumanoidStateType.Freefall or state == Enum.HumanoidStateType.Jumping then
                    canJump = false
                    hum:ChangeState(Enum.HumanoidStateType.Jumping)
                    task.spawn(function()
                        -- Mencegah lompat terus-menerus jika tombol spasi ditahan (PC)
                        if not UserInputService:GetFocusedTextBox() and UserInputService:IsKeyDown(Enum.KeyCode.Space) then
                            ...
                            task.wait(0.2) -- Batas jeda waktu sentuh untuk mobile
                        end
                        canJump = true
                    end)
                end
